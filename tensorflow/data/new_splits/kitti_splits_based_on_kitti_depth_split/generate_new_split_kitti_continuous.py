...
class KittiContinuous:
    ...
    def get_image_depth_pair_relative_paths(self):
        file_path = 'kitti_continuous.txt'
        ...
        if os.path.exists(file_path):
            ...
            np.savetxt('kitti_continuous.txt', self.pairs, fmt='%s', delimiter='\t')
...
def main():
    ...
    kitti_continuous.get_image_depth_pair_relative_paths()
    kitti_continuous.get_filenames()
    ...
    print()
    print('kitti_continuous_pairs:', len(kitti_continuous.pairs))
    print('kitti_continuous_filenames:', len(kitti_continuous.filenames))
    print()

    # TODO: Melhorar isto
    kitti_depth.filenames = kitti_depth.train.filenames + kitti_depth.test.filenames

    np.savetxt('kitti_continuous_sorted.txt', sorted(kitti_continuous.filenames), fmt='%s', delimiter='\t')
    ...
    for item in set(kitti_continuous.filenames):
        if item in kitti_depth.train.filenames:
            ...
            pair = [s for s in kitti_continuous.pairs if item in s]
            # print(pair)
    ...
    print('| {:9}\t|\t{:9}|'.format(len(kitti_continuous.filenames), len(kitti_continuous.train.new_split)))
    print('| {:9}\t|\t{:9}|'.format('', len(kitti_continuous.test.new_split)))
    print('# ---------------------- #\n')
