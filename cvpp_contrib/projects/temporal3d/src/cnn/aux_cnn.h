...
void process_frame( const SeqPosef& poses , const SeqMatd& vels , const int& t ,
                    const Matf& TRP , const Matf& iPRT ,const int& r , const int& c ,
                    Matf& vel , Matf& xyz , Matf& proj , Matf& uv , Matf& dep )
{
    vel = poses[t].o2w( vels[t].cl(3).toFloat() );

//    proj = poses[t].w2o( vel ).appR1() * TRP;
    proj = vels[t].cl(3).toFloat().appR1() * TRP;

    filter_img( proj , r , c );

//    dep = proj.c(2) , uv = proj.cl(2) / dep;
//    dep.AddRand( -0.5 , +0.5 );
//    xyz = poses[t].o2w( ( ( uv.appR1() % dep ).appR1() * iPRT ).cl(3) );

    dep = proj.c(2) , uv = proj.cl(2) / dep;
    dep.AddRand( 0 , +0.25 );
    Matf xyz1 = poses[t].o2w( ( ( uv.appR1() % ( dep        ) ).appR1() * iPRT ).cl(3) );
    Matf xyz2 = poses[t].o2w( ( ( uv.appR1() % ( dep + 0.25 ) ).appR1() * iPRT ).cl(3) );
    Matf xyz3 = poses[t].o2w( ( ( uv.appR1() % ( dep + 0.50 ) ).appR1() * iPRT ).cl(3) );
    Matf xyz4 = poses[t].o2w( ( ( uv.appR1() % ( dep + 0.75 ) ).appR1() * iPRT ).cl(3) );
    Matf xyz5 = poses[t].o2w( ( ( uv.appR1() % ( dep + 1.00 ) ).appR1() * iPRT ).cl(3) );
    Matf xyz6 = poses[t].o2w( ( ( uv.appR1() % ( dep + 1.25 ) ).appR1() * iPRT ).cl(3) );
    Matf xyz7 = poses[t].o2w( ( ( uv.appR1() % ( dep + 1.50 ) ).appR1() * iPRT ).cl(3) );

    xyz = xyz1 | xyz2 | xyz3 | xyz4 | xyz5 | xyz6 | xyz7;
}
                    ...
                    Img1f& dspf , Img1c& dspc , const double& mlt = 3.0 )
{
    ...
    forLOOPj( uv.r() )
    {
        ...
        int v = std::floor( uv(j,1) );
        int u = std::floor( uv(j,0) );

        if( dspf(v,u) == 0 ) dspf(v,u) = val;
        else if( dspf(v,u) > val ) dspf(v,u) = val;
    }
    dspc = dspf.toUChar();
}
...
void prep_dirs( const String& dir )
{
    ...
    path = dir + "/proc";       if( stat( path.c_str() , &st ) == -1 ) mkdir( path.c_str() , 0700 );
    path = dir + "/proc/imgs";  if( stat( path.c_str() , &st ) == -1 ) mkdir( path.c_str() , 0700 );
    path = dir + "/proc/dispc"; if( stat( path.c_str() , &st ) == -1 ) mkdir( path.c_str() , 0700 );
    path = dir + "/proc/dispf"; if( stat( path.c_str() , &st ) == -1 ) mkdir( path.c_str() , 0700 );
    path = dir + "/proc/vel";   if( stat( path.c_str() , &st ) == -1 ) mkdir( path.c_str() , 0700 );
    path = dir + "/proc/xyz";   if( stat( path.c_str() , &st ) == -1 ) mkdir( path.c_str() , 0700 );
    path = dir + "/proc/uv";    if( stat( path.c_str() , &st ) == -1 ) mkdir( path.c_str() , 0700 );
    path = dir + "/proc/dep";   if( stat( path.c_str() , &st ) == -1 ) mkdir( path.c_str() , 0700 );
}

void save_data( const String& suf , const String& dir , const String& path ,
                const Img3c& img , const Img1f& dspf , const Img1c& dspc ,
                const Matf& vel , const Matf& xyz , const Matf& uv , const Matf& dep )
{    
    ...
    img.saveIMG( dir + "/proc/imgs/" + sufname + ".png" );
    dspc.saveIMG( dir + "/proc/dispc/" + sufname + ".png" );
    dspf.mat().saveBIN( dir + "/proc/dispf/" + sufname + ".bin" );
    vel.saveBIN( dir + "/proc/vel/" + sufname + ".bin" );
    xyz.saveBIN( dir + "/proc/xyz/" + sufname + ".bin" );
    uv.saveBIN(  dir + "/proc/uv/"  + sufname + ".bin" );
    dep.saveBIN( dir + "/proc/dep/" + sufname + ".bin" );
}
